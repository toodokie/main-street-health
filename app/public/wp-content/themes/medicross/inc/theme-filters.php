...
function medicross_add_cpt_support() {
    ...
    if( ! $cpt_support ) {
        $cpt_support = [ 'page', 'post', 'portfolio', 'service','industries', 'pxl_product', 'footer', 'pxl-template' ];
    ...
    else if( ! in_array( 'pxl_product', $cpt_support ) ) {
        $cpt_support[] = 'pxl_product';
        update_option( 'elementor_cpt_support', $cpt_support );
    }
...
/* Flush rewrite rules on theme activation for new post types */
add_action('after_switch_theme', 'medicross_flush_rewrite_rules');
function medicross_flush_rewrite_rules() {
    flush_rewrite_rules();
}

/* One-time flush for new Products post type - remove after first load */
add_action('init', 'medicross_flush_rewrite_rules_once', 999);
function medicross_flush_rewrite_rules_once() {
    if (!get_option('medicross_products_rewrite_flushed')) {
        flush_rewrite_rules();
        update_option('medicross_products_rewrite_flushed', true);
    }
}

/* Force refresh Elementor post type support for Products */
add_action('init', 'medicross_force_elementor_support', 1000);
function medicross_force_elementor_support() {
    if (!get_option('medicross_elementor_support_refreshed')) {
        $cpt_support = get_option('elementor_cpt_support', []);
        
        // Force add our post type if not present
        if (!in_array('pxl_product', $cpt_support)) {
            $cpt_support[] = 'pxl_product';
            update_option('elementor_cpt_support', $cpt_support);
        }
        
        // Mark as refreshed
        update_option('medicross_elementor_support_refreshed', true);
        
        // Force Elementor to refresh its settings
        if (class_exists('\Elementor\Plugin')) {
            delete_option('elementor_editor_upgrade_has_access');
...
function medicross_add_post_type( $postypes ) {
	$theme_options = get_option(medicross()->get_option_name(), []);
	...
	$portfolio_name = !empty( $theme_options['portfolio_name'] ) ? $theme_options['portfolio_name'] : esc_html__( 'Portfolio', 'medicross' );
	...
	$service_name = !empty( $theme_options['service_name'] ) ? $theme_options['service_name'] : esc_html__( 'Service', 'medicross' );
	...
	$industries_name = !empty( $theme_options['industries_name'] ) ? $theme_options['industries_name'] : esc_html__( 'Industries', 'medicross' );

	// Add Products and Devices custom post type
	$product_display = medicross()->get_theme_opt('product_display', true);
	$product_slug = !empty( $theme_options['product_slug'] ) ? $theme_options['product_slug'] : 'products-devices'; 
	$product_name = !empty( $theme_options['product_name'] ) ? $theme_options['product_name'] : esc_html__( 'Products & Devices', 'medicross' );
	...
	// Add Products & Devices custom post type (non-WooCommerce)
	$postypes['pxl_product'] = array(
...
function medicross_add_tax( $taxonomies ) {
	...
	$industries_categorie_slug = medicross()->get_theme_opt('industries_categorie_slug', 'industries');
	$industries_categorie_name = medicross()->get_theme_opt('industries_categorie_name', 'Industries');
	...
	// Add Product & Device Categories taxonomy
	$taxonomies['pxl-product-category'] = array(
		'status'     => true,
		'post_type'  => array( 'pxl_product' ),
		'taxonomy'   => 'Product & Device Categories',
		'taxonomies' => 'Product & Device Categories',
		'args'       => array(
			'rewrite'             => array(
                'slug'       => 'product-device-category'
 		 	),
		),
		'labels'     => array()
	);
