...
function medicross_scripts() {  
    ...
    /* Elementor Typography Override - Load after main styles to allow Elementor control */
    wp_enqueue_style( 'pxl-elementor-override', get_template_directory_uri() . '/assets/css/custom-elementor-override.css', array( 'pxl-style' ), $medicross_version->get( 'Version' ) );
    ...
    if (is_post_type_archive('pxl_product') || is_singular('pxl_product') || is_tax('pxl-product-category')) {
        wp_enqueue_style( 'pxl-products', get_template_directory_uri() . '/assets/css/products.css', array( 'pxl-style' ), $medicross_version->get( 'Version' ) );
    }
    
    /* MSH Accordion Widget Assets - Register for conditional loading */
    wp_register_style( 'pxl-msh-accordion', get_template_directory_uri() . '/assets/css/msh-accordion.css', array( 'pxl-style' ), time() );
    wp_register_script( 'pxl-msh-accordion', get_template_directory_uri() . '/assets/js/msh-accordion.js', array(), time(), true );
...
if(!function_exists('medicross_hook_anchor_search')){
    function medicross_hook_anchor_search(){
        $logo_s_p = medicross()->get_theme_opt( 'logo_s_p', ['url' => get_template_directory_uri().'/assets/img/logo.png', 'id' => '' ] ); 
        $placeholder_search_pu = medicross()->get_theme_opt( 'placeholder_search_pu', 'Search...'); 
        ?>
