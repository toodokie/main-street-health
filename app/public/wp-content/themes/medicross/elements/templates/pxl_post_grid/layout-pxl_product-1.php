...
$post_type = $widget->get_setting('post_type', 'pxl_product');
$layout = $widget->get_setting('layout_'.$post_type, 'pxl_product-1');
...
$wrap_attrs = [
    'id'               => $html_id,
    'class'            => trim('pxl-grid pxl-product-grid pxl-product-grid-layout1 pxl-product-style1 '),
        ...
        <div class="wrap-filter">
                        ...
                        foreach ($categories as $category) {
                            ...
                            if (!empty($posts) && is_array($posts)) {
                                foreach ($posts as $key => $post) {
                                    $this_terms = get_the_terms($post->ID, 'pxl-product-category');
                                    $term_list = [];
            ...
            <?php if($settings['search'] == 'true') { ?>
                <div class="pxl-grid--search">
                    <input type="text" class="grid-search-input" placeholder="Search Products...">
    ...
    <?php if (!empty($next_link) && $pagination_type == 'loadmore') { ?>
        <div class="pxl-load-more">
            <span class="btn-grid-loadmore ">
                <span class="pxl-loadmore-text" data-text="Learn More Products">
                    <?php if (!empty($settings['button_text_load_more'])) {
                        echo pxl_print_html($settings['button_text_load_more']);
                    }else {
                        echo esc_html__('Load More Products', 'medicross'); 
                    }?>
