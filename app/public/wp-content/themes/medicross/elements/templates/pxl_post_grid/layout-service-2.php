...
// Determine post type and taxonomy dynamically
$post_type = $widget->get_setting('post_type', 'service');
if ($post_type === 'pxl_product') {
    // Use product taxonomy for filtering, but keep Service CSS classes so styling matches layout #2 exactly
    $tax = ['pxl-product-category'];
    $css_class_base = 'pxl-service-grid pxl-service-grid-layout2 pxl-service-style1';
    $no_posts_message = 'No Products Found';
    ...
    $load_more_text = 'Load More Projects';
}
...
if ($post_type === 'pxl_product') {
    // Force Service layout #2 for Products/Devices
    $layout = 'service-2';
}
...
$product_title_tag = $widget->get_setting('product_title_tag');
...
$wrap_attrs = [
    'id'               => $html_id,
    'class'            => trim('pxl-grid ' . $css_class_base . ' '),
...
if( count($posts) <= 0){
    echo '<div class="pxl-no-post-grid">'.esc_html__( $no_posts_message, 'medicross' ). '</div>';
    return;
} ?>
        ...
        <div class="wrap-filter">
                        ...
                        foreach ($categories as $category) {
                            ...
                            if (!empty($posts) && is_array($posts)) {
                                foreach ($posts as $key => $post) {
                                    $this_terms = get_the_terms($post->ID, $tax[0]);
                                    $term_list = [];
            ...
            <?php if($settings['search'] == 'true') { ?>
                <div class="pxl-grid--search">
                    <input type="text" class="grid-search-input" placeholder="<?php echo esc_attr($search_placeholder); ?>">
