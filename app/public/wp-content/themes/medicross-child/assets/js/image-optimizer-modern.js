...
(function($) {
    ...
    const STATUS_DEFINITIONS = {
        'optimized': { label: 'Optimized', color: '#daff00', icon: '✅' },
        'needs_webp_conversion': { label: 'Needs WebP', color: '#FF9800', icon: '🔄' },
        'webp_timestamp_missing': { label: 'WebP Fix Needed', color: '#FFC107', icon: '⚠️' },
        'ready_for_optimization': { label: 'Needs Optimization', color: '#F44336', icon: '❌' },
        'metadata_missing': { label: 'Needs Metadata', color: '#E91E63', icon: '📝' },
        'needs_recompression': { label: 'Needs Update', color: '#9C27B0', icon: '🔄' },
        'webp_missing': { label: 'WebP Missing', color: '#FF5722', icon: '🚫' },
        'webp_optimized': { label: 'WebP Active', color: '#4CAF50', icon: '📦' },
        'needs_attention': { label: 'Attention Required', color: '#607D8B', icon: '⚠️' }
    };
    ...
    const AppState = {
        images: [],
        filters: {
            status: 'needs_optimization',  // Default to needs_optimization to match HTML
    ...
    class UI {
        ...
        static setupEventListeners() {
            ...
            $('#optimize-all').on('click', () => Optimization.runAll());
            $('#optimize-high-priority').on('click', () => Optimization.runByPriority('high'));
            $('#optimize-medium-priority').on('click', () => Optimization.runByPriority('medium'));
        ...
        static updateSelectAllState() {
            ...
            $('#optimize-high-priority, #optimize-medium-priority, #optimize-all').prop('disabled', !hasResults);
        }
        ...
        static renderImageRow(image) {
            ...
            const priorityClass = this.getPriorityClass(image.priority);
            const issuesText = (image.issues || []).map(i => i.label).join(', ') || 'None';
            ...
            return `
                <tr class="result-row ${priorityClass}" data-attachment-id="${image.ID}">
                    <td>
                        <input type="checkbox" class="image-select" value="${image.ID}" />
                    </td>
                    <td class="image-cell">
                        <img src="${thumbnailUrl}" alt="${altText}" style="width: 50px; height: 50px; object-fit: cover;" />
                    </td>
                    ...
                    <td class="priority-cell">
                        <span class="priority-badge priority-${priorityClass}">${image.priority}</span>
                    </td>
                    <td class="status-cell">
                        <span class="status-badge" style="background-color: ${status.color};">
                            ${status.icon} ${status.label}
                        </span>
                        <br><small>${issuesText}</small>
                    </td>
                    <td class="size-cell">
        ...
        static renderWebPInfo(image) {
            ...
            if (savings && savings.estimated_savings_percent > 0) {
                return `<br><small style="color: #4CAF50;">📦 WebP delivery active (-${savings.estimated_savings_percent}%)</small>`;
            } else {
                return '<br><small style="color: #4CAF50;">📦 WebP delivery active</small>';
            }
        }
        ...
        static renderFilenameSuggestion(image) {
            ...
            return `
                <div style="margin-top: 8px; padding: 8px; background: #f8f9fa; border-left: 3px solid #0073aa; border-radius: 4px; font-size: 12px; max-width: 100%; box-sizing: border-box;">
                    <div style="color: #0073aa; font-weight: 500; margin-bottom: 4px;">💡 Suggested filename:</div>
                    <div class="suggestion-filename-display" data-id="${image.ID}" style="color: #2c3338; margin-bottom: 6px; word-break: break-all; font-family: monospace; font-size: 11px;">${this.escapeHtml(image.suggested_filename)}</div>
                    <input type="text" class="suggestion-filename-edit" data-id="${image.ID}" value="${this.escapeHtml(image.suggested_filename)}" style="display: none; width: 100%; margin-bottom: 6px; font-family: monospace; font-size: 11px; padding: 4px;" />
                    <div style="display: flex; gap: 6px; flex-wrap: wrap;">
                        <button class="button button-small apply-suggestion" data-id="${image.ID}" style="font-size: 11px; padding: 2px 8px; background: #0073aa; color: white; border: none; border-radius: 3px; cursor: pointer;">Apply</button>
                        <button class="button button-small edit-suggestion" data-id="${image.ID}" style="font-size: 11px; padding: 2px 8px; background: #f0f0f1; color: #2c3338; border: 1px solid #c3c4c7; border-radius: 3px; cursor: pointer;">Edit</button>
                        <button class="button button-small save-suggestion" data-id="${image.ID}" style="display: none; font-size: 11px; padding: 2px 8px; background: #00a32a; color: white; border: none; border-radius: 3px; cursor: pointer;">Save</button>
                        <button class="button button-small cancel-suggestion" data-id="${image.ID}" style="display: none; font-size: 11px; padding: 2px 8px; background: #f0f0f1; color: #2c3338; border: 1px solid #c3c4c7; border-radius: 3px; cursor: pointer;">Cancel</button>
                        <button class="button button-small reject-suggestion" data-id="${image.ID}" style="font-size: 11px; padding: 2px 8px; background: #f0f0f1; color: #2c3338; border: 1px solid #c3c4c7; border-radius: 3px; cursor: pointer;">Dismiss</button>
                    </div>
                </div>
            `;
        }
        ...
        static renderMetaPreview(image) {
            ...
            return `
                <details class="meta-preview" style="margin-top: 8px;">
                    <summary style="cursor: pointer; color: #35332f; font-size: 11px;">📝 Meta Preview</summary>
                    <div style="margin-top: 5px; padding: 8px; background: #faf9f6; border: 1px solid #35332f; border-radius: 4px; font-size: 11px;">
                        ${metaFields.join('<br>')}
                        <div style="margin-top: 5px;">
                            <button class="button button-small edit-meta" data-id="${image.ID}" style="font-size: 10px;">Edit</button>
                        </div>
                    </div>
                </details>
            `;
        ...
        static renderContextDropdown(image) {
            ...
            if (contextSource === 'manual') {
                statusBadge = '<span style="background: #0073aa; color: white; padding: 2px 6px; border-radius: 3px; font-size: 10px; margin-left: 5px;">Manual</span>';
            } else {
                statusBadge = '<span style="background: #f0f0f1; color: #2c3338; padding: 2px 6px; border-radius: 3px; font-size: 10px; margin-left: 5px;">Auto</span>';
            }

            return `
                <div class="context-control">
                    <div style="margin-bottom: 4px; font-size: 12px; color: #35332f;">
                        <strong>${this.escapeHtml(activeLabel)}</strong>
                        ${statusBadge}
                    </div>
                    <select class="context-dropdown" data-attachment-id="${image.ID}" style="width: 100%; font-size: 11px; padding: 3px;">
                        ${optionsHTML}
                    </select>
                    ${autoLabel ? `<div style="font-size: 10px; color: #666; margin-top: 3px;">Auto: ${this.escapeHtml(autoLabel)}</div>` : ''}
                </div>
            `;
        }
        ...
        static showMetaEditModal(attachmentId) {
            ...
            const modalHTML = `
                <div id="meta-edit-modal" style="position: fixed; top: 0; left: 0; width: 100%; height: 100%; background: rgba(0,0,0,0.5); z-index: 10000; display: flex; align-items: center; justify-content: center;">
                    <div id="meta-edit-content" style="background: white; padding: 20px; border-radius: 8px; max-width: 600px; width: 90%; max-height: 80%; overflow-y: auto;">
                        <h3>Edit Meta for: ${image.post_title || 'Untitled'}</h3>
                        <form id="meta-edit-form">
                            <div style="margin-bottom: 15px;">
                                <label><strong>Title:</strong></label>
                                <input type="text" id="edit-title" value="${this.escapeHtml(meta.title || '')}" style="width: 100%; padding: 8px; margin-top: 5px;">
                            </div>
                            <div style="margin-bottom: 15px;">
                                <label><strong>ALT Text:</strong></label>
                                <input type="text" id="edit-alt" value="${this.escapeHtml(meta.alt_text || '')}" style="width: 100%; padding: 8px; margin-top: 5px;">
                            </div>
                            <div style="margin-bottom: 15px;">
                                <label><strong>Caption:</strong></label>
                                <textarea id="edit-caption" rows="3" style="width: 100%; padding: 8px; margin-top: 5px;">${this.escapeHtml(meta.caption || '')}</textarea>
                            </div>
                            <div style="margin-bottom: 15px;">
                                <label><strong>Description:</strong></label>
                                <textarea id="edit-description" rows="4" style="width: 100%; padding: 8px; margin-top: 5px;">${this.escapeHtml(meta.description || '')}</textarea>
                            </div>
                            <div style="text-align: right;">
                                <button type="button" id="cancel-meta-edit" class="button button-secondary" style="margin-right: 10px;">Cancel</button>
                                <button type="submit" id="save-meta-edit" class="button button-primary">Save Changes</button>
                            </div>
                        </form>
                    </div>
                </div>
        ...
        static updateStats() {
            ...
            // Update buttons
            $('#optimize-high-priority').prop('disabled', !images.some(img =>
                img.priority >= 15 && img.optimization_status !== 'optimized'
            ));
            $('#optimize-medium-priority').prop('disabled', !images.some(img =>
                img.priority >= 10 && img.priority < 15 && img.optimization_status !== 'optimized'
            ));
            $('#optimize-all').prop('disabled', remaining === 0);
        }
